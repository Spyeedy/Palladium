...
public class ParticleEmitter {
    ...
    public ParticleEmitter(@Nullable BodyPart anchor, int amount, Vector3f offset, Vector3f offsetRandom, Vector3f motion, Vector3f motionRandom) {
        this.anchor = anchor;
        this.amount = amount;
        this.offset = offset;
    ...
    public static ParticleEmitter fromJson(JsonObject json) {
        ...
        return new ParticleEmitter(bodyPart, amount, offset, offsetRandom, motion, motionRandom);
    }
    ...
    public void spawnParticle(Level level, AbstractClientPlayer player, ParticleOptions particleOptions, float partialTick) {
        ...
        for (int i = 0; i < this.amount; i++) {
            ...
            var motion = new Vector3f(this.motion).add(
                    (random.nextFloat() - 0.5F) * 2F * this.motionRandom.x,
                    (random.nextFloat() - 0.5F) * 2F * this.motionRandom.y,
                    (random.nextFloat() - 0.5F) * 2F * this.motionRandom.z);
