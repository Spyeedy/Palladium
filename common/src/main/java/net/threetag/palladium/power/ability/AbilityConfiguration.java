...
public class AbilityConfiguration {
    ...
    public static final Codec<AbilityConfiguration> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    PalladiumRegistries.ABILITY_SERIALIZER.byNameCodec().fieldOf("type").forGetter(AbilityConfiguration::getAbility),
                    CompoundTag.CODEC.optionalFieldOf("properties", new CompoundTag()).forGetter(config -> config.getPropertyManager().toNBT(false)),
                    AbilityConditions.CODEC.optionalFieldOf("conditions", AbilityConditions.EMPTY).forGetter(AbilityConfiguration::getConditions),
    ...
    private String key;
    private final AbilitySerializer ability;
    private final PropertyManager propertyManager;
    private final AbilityConditions conditions;
    private final List<EnergyBarUsage> energyBarUsages;

    public AbilityConfiguration(AbilitySerializer ability, CompoundTag rawProperties, AbilityConditions conditions, List<EnergyBarUsage> energyBarUsages) {
    ...
    public AbilitySerializer getAbility() {
        return ability;
    }

    public Component getDisplayName() {
        Component title = this.propertyManager.get(AbilitySerializer.TITLE);
        ResourceLocation id = PalladiumRegistries.ABILITY_SERIALIZER.getKey(this.getAbility());
